...
def create_resource(resource: schemas.ResourceCreate, db: Session = Depends(get_db)):
    return orders.create(db=db, resource=resource)
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    resource = orders.read_one(db, resource_id=resource_id)
    if resource is None:
        raise HTTPException(status_code=404, detail="User not found")
    return resource
...
def update_one_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    resource_db = orders.read_one(db, resource_id=resource_id)
    if resource_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orders.update(db=db, resource=resource, resource_id=resource_id)
...
def delete_one_resource(resource_id: int, db: Session = Depends(get_db)):
    ...
    if resource is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orders.delete(db=db, resource_id=resource_id)
...
def create_order_detail(order_detail: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return orders.create(db=db, order_detail=order_detail)
...
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = orders.read_one(db, order_detail_id=order_detail_id)
...
def update_one_order_detail(order_detail_id: int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    order_detail_db = orders.read_one(db, order_detail_id=order_detail_id)
    if order_detail_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orders.update(db=db, order_detail=order_detail, order_detail_id=order_detail_id)
...
def delete_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    ...
    if order_detail is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orders.delete(db=db, order_detail_id=order_detail_id)
